...
| --- | --- | --- | --- | --- |
| Duplicate delivery | Same identity, same hash: HTTP `duplicate`; one stored event; one economic transition | `apps/api/src/failure-lab/scenarios.e2e.test.ts` (`DUPLICATE`); ingest uniqueness tests in `@hookx/storage` | No second capture/create | `WEBHOOK_DUPLICATE`; `DUPLICATE_EVENT` exception |
