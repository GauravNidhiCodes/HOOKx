...
| Golden Demo | Unique synthetic Razorpay-shaped authorized webhook; signature; normalize; ingest; fail-once; retry succeeds; duplicate redelivery; one stored event; audit; incident timeline; stub investigation; no second economic effect | `apps/api/src/demo/golden.e2e.test.ts`; `apps/web/src/pages/Demo.test.tsx` | Retry `SUCCEEDED`; event `PROCESSED`; payment projection empty | Retry + `PROCESSING_FAILURE` + duplicate audit |
| Golden Demo exhaustion | Razorpay-shaped lab payment with `ALWAYS_FAIL` | `apps/api/src/demo/exhaustion.e2e.test.ts` | Dead-letter; UI would show **RETRY EXHAUSTED**, not recovery | `RETRY_DEAD_LETTERED`; `RETRY_EXHAUSTED` |
